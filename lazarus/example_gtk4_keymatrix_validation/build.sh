#!/bin/bash
# build.sh <gtk4|gtk2|qt5> -> binary keymatrix_<ws>
set -e
...
rm -rf $S/lib/$WS; mkdir -p $S/lib/$WS; rm -f $S/keymatrix_$WS; cd $S
for i in 1 2; do
  fpc -MObjFPC -Scghi -Cg -O1 -g -gl -l -vewnhibq -dLCL $DEF \
    -Fu$LCLU/$WS -Fu$LCLU -Fu$LAZU -Fu$FT -Fu$PK -Fi$LCLU/../../include -Fi$LCLU/../.. -Fi$LCLU/../../interfaces/$WS \
    -FU$S/lib/$WS -FE$S -o$S/keymatrix_$WS $S/keymatrix.lpr > $S/build_$WS.log 2>&1 && break
done
grep -E 'Error|Fatal|lines compiled' $S/build_$WS.log | head -5
test -x $S/keymatrix_$WS || { echo 'BUILD FAILED'; exit 1; }
