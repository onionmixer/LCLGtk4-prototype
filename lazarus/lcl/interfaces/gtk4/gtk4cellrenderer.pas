...
uses
  Classes, SysUtils, LCLType, LCLProc, LazLoggerBase, Controls, StdCtrls, ComCtrls, LMessages,
  LazGObject2, LazGtk4, LazGdk4, LazGLib2, Gtk4Procs, LazCairo1;
...
begin
  ...
  if AHWND <> 0 then begin
    Result := TGtk4Widget(AHWND).LCLObject;
    Assert(Result is TWinControl, 'GetControl: LCLObject for Gtk4Widget is not TWinControl.');
  end;
end;
...
begin
  ...
  TGtk4Widget(AWinControl.Handle).DeliverMessage(Msg);
...
var
  ...
  MeasureItemStruct: TMeasureItemStruct;
begin
  // THIS DOES NOT WORK IN Gtk4 ANYMORE.
  {$IFDEF GTK4DEBUGCELLRENDERER}
...
begin
  ...
  begin
    ...
    if Widget = TGtk4Widget(AWinControl.Handle).Widget then
      DebugLn(' MAIN WIDGET ...')
    else
    if Widget = TGtk4Widget(AWinControl.Handle).GetContainerWidget then
    begin
      DebugLn(' CONTAINER WIDGET ... OK ');
      if wtListBox in TGtk4Widget(AWinControl.Handle).WidgetType then
        ColumnIndex := -1;
      if TGtk4Widget(AWinControl.Handle).Context <> 0 then
        DebugLn(' CONTAINER WIDGET ... HAVE CONTEXT !!!!! ');
    end
    else
    begin
      DebugLn(' FATAL ERROR DONT KNOW WHAT WIDGET IS THIS !');
      if gtk_widget_get_parent(Widget) = TGtk4Widget(AWinControl.Handle).Widget then
        DebugLn(' BUT PARENT IS MAIN WIDGET !')
      else
      if gtk_widget_get_parent(Widget) = TGtk4Widget(AWinControl.Handle).GetContainerWidget then
      begin
        DebugLn(' BUT PARENT IS CONTAINER WIDGET ! CellView=',dbgs(Gtk4IsCellView(Widget)),' w=',dbgHex(PtrUInt(Widget)));
        if Gtk4IsCellView(Widget) then
        begin
  ...
  *)
  ...
  begin
    ...
    begin
      // OwnerDraw ListView: send CN_DRAWITEM instead of IntfCustomDraw
      Msg.Msg := CN_DRAWITEM;

      // collect state flags
      State := [];
      if GTK_CELL_RENDERER_SELECTED in flags then
        Include(State, odSelected);
      if not Widget^.is_sensitive then
        Include(State, odInactive);
      if Widget^.has_default then
        Include(State, odDefault);
      if GTK_CELL_RENDERER_FOCUSED in flags then
      ...
      try
        ...
        begin
          ...
          Area := AreaRect;
          DCWidget := Widget;
          DC := Gtk4WidgetSet.CreateDCForWidget(DCWidget, nil, cr);
          ItemState := State;
        end;
        TGtk4Widget(AWinControl.Handle).DeliverMessage(Msg);
        Gtk4WidgetSet.ReleaseDC(AWinControl.Handle, Msg.DrawListItemStruct^.DC);
      finally
    ...
    DCWidget := Widget;
    TmpDC1 := Gtk4WidgetSet.CreateDCForWidget(DCWidget, nil, cr);
    ...
    begin
      Gtk4WidgetSet.ReleaseDC(TCustomListViewAccess(AWinControl).Handle,TmpDC1);
...
begin
  ...
  if (wtComboBox in TGtk4Widget(Data).WidgetType) and
    not (TCustomComboBox(TGtk4Widget(Data).LCLObject).Style.HasEditBox) and
    not (TCustomComboBox(TGtk4Widget(Data).LCLObject).DroppedDown) then
  begin
    Value.clear;
  ...
  if wtListView in TGtk4Widget(Data).WidgetType then
  begin
    gtk_tree_model_get(tree_model, iter, [0, @ListItem, -1]);
    if (ListItem = nil) and TCustomListView(TGtk4Widget(Data).LCLObject).OwnerData then
      ListItem := TCustomListView(TGtk4Widget(Data).LCLObject).Items[LCLCellRenderer^.Index];
  ...
  if (wtListBox in TGtk4Widget(Data).WidgetType) then
  begin
    if TGtk4ListBox(Data).ListBoxStyle < lbOwnerDrawFixed then
    begin
      value.clear;
      value.init(G_TYPE_STRING);
      ...
      S := TCustomListBox(TGtk4Widget(Data).LCLObject).Items.Strings[LCLCellRenderer^.Index];
      value.clear;
      value.init(G_TYPE_STRING);
      Value.set_string(PgChar(S));
