...
// use scAll to update a statusbar when another TCustomSynEdit got the focus
  ...
  {$IFDEF Gtk4IME}
  protected
    procedure GTK4_IMComposition(var Message: TMessage); message LM_IM_COMPOSITION;
  {$ENDIF}
  {$IFDEF CocoaIME}
  private
...
begin
  ...
  {$ENDIF}
  {$IFDEF Gtk4IME}
  FImeHandler := LazSynImeGtk4.Create(Self);
  FImeHandler.InvalidateLinesMethod := @InvalidateLines;
  {$ENDIF}
...
{$ifdef Gtk4IME}
procedure TCustomSynEdit.GTK4_IMComposition(var Message: TMessage);
begin
  FImeHandler.WMImeComposition(Message);
end;
{$endif}
