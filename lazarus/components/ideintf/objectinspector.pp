...
var
  CurGrid: TOICustomPropertyGrid;
begin
  { Do not intercept keys when filter edits have focus — let them type normally.
    ActiveControl is the internal TEbEdit child, not the filter edit container,
    so check via IsParentOf to match children at any depth. }
  if (ActiveControl <> nil) and
     (((CompFilterEdit <> nil) and CompFilterEdit.IsParentOf(ActiveControl)) or
      ((PropFilterEdit <> nil) and PropFilterEdit.IsParentOf(ActiveControl))) then
  begin
    inherited KeyDown(Key, Shift);
    exit;
  end;
